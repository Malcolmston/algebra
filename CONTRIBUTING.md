...
subset of Python's SymPy, so contributions that improve correctness, coverage,
tests, or docs are especially welcome.
...
5. Favour **correctness over coverage**: an operation that cannot transform an
   expression should return it unchanged (or an explicit unevaluated node)
